...
function baseUrl(path: string): string | null {
...
function snapshotUrl(slot: SnapshotSlot, path: string): string | null {
  const normalized = path.replace(/^\/data\//, "").replace(/^\//, "");
  return baseUrl(`/data/${slot}/${normalized}`);
}

async function readSlotJson<T>(slot: SnapshotSlot, path: string, revalidateSeconds: number): Promise<T | null> {
  const url = snapshotUrl(slot, path);
  if (!url) return null;
  try {
    ...
    console.error(`[publishing] ${slot} build snapshot unavailable: ${path}`, error);
    return null;
  }
}
...
async function readJsonWithFallback<T>(
  ...
  const result = await readWithBuildFallback<T>(
    (slot) => readSlotJson<T>(slot, path, revalidateSeconds),
    validate,
  );
  if (result.slot === "previous") {
    console.warn(`[publishing] CURRENT invalid/unavailable; using PREVIOUS build snapshot: ${path}`);
  }
  return result.value;
}

async function readLiveJson<T>(path: string, revalidateSeconds: number): Promise<T | null> {
  ...
  try {
    ...
    console.error(`[publishing] live snapshot unavailable: ${path}`, error);
    return null;
  }
}
...
async function loadAirports(): Promise<AirportIdentityV1[]> {
  const snapshot = await readJsonWithFallback<SnapshotEnvelope<AirportIdentityV1>>(
    "/data/v1/willitfit-airports.v1.json",
    3600,
    (value) => value.contractVersion === "1.0.0" && Array.isArray(value.rows),
  );
  if (!snapshot) return [];
...
async function loadAirportReferences(): Promise<AirportReferenceV1[]> {
  const snapshot = await readJsonWithFallback<SnapshotEnvelope<AirportReferenceV1>>(
...
async function loadAviation(): Promise<AviationCurrentV1 | null> {
  const snapshot = await readLiveJson<AviationCurrentV1>("/data/live/aviation-current.v1.json", 60);
...
async function loadCommercial(): Promise<CommercialSnapshotV1 | null> {
  const snapshot = await readJsonWithFallback<CommercialSnapshotV1>(
    "/data/v1/willitfit-commercial.v1.json",
    300,
    (value) => value.contractVersion === "1.0.0" && Array.isArray(value.placements),
  );
