...
function sortedDimensions(rule: Rc6AirlineRule): readonly number[] | null {
  ...
  return [
    rule.sizingRule.dimensions.heightCm,
    rule.sizingRule.dimensions.widthCm,
    rule.sizingRule.dimensions.depthCm,
  ].sort((a, b) => a - b);
}
...
function sizingNoMorePermissive(left: Rc6AirlineRule, right: Rc6AirlineRule): boolean {
  ...
  if (left.sizingRule.method === "weight-only" && right.sizingRule.method === "weight-only") {
    return true;
  }
...
export function resolveRc6CheckerRule(
  rules: readonly Rc6AirlineRule[],
  airlineId: string,
  bagType: Rc6BagType,
  fare?: string | null,
  ...
  if (candidates.length === 0) {
    return { state: "UNAVAILABLE", rule: null, fare: null, availableFares };
  }
  ...
  if (dominant.length === 0) {
    return { state: "FARE_REQUIRED", rule: null, fare: null, availableFares };
  }

  return { state: "RESOLVED", rule: dominant[0]!, fare: dominant[0]!.fare };
}

export function rc6AvailableBagTypes(
  rules: readonly Rc6AirlineRule[],
  airlineId: string,
): Rc6BagType[] {
...
export async function getRc6CheckerCatalogue(reader: Rc6TabReader): Promise<Rc6CheckerCatalogue | null> {
  const [airlines, rules] = await Promise.all([
    getRc6Airlines(reader),
    getRc6AirlineRules(reader),
  ]);

  if (airlines.length !== 114 || rules.length !== 425) return null;
...
export function assessRc6Checker(input: Readonly<{
  ...
  const airline = input.catalogue.airlines.find(
    (candidate) => candidate.airlineId === input.airlineId.trim().toUpperCase(),
  );
  if (!airline) {
    return { state: "UNAVAILABLE", rule: null, fare: null, availableFares: [] };
  }

  const resolution = resolveRc6CheckerRule(
    input.catalogue.rules,
    airline.airlineId,
    input.bagType,
    input.fare,
  );
