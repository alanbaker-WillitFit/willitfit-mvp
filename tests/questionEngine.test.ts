...
describe("RC15 Question Engine", () => {
  ...
  it.each(["fits", "close", "no-fit"] as const)(
    "returns no more than three questions for %s",
    (verdict) => {
  ...
  it("applies the measurement fallback and ranks Q-0001 first", () => {
    expect(getResultQuestions(makeResult("fits"))[0]?.questionId).toBe(
      "Q-0001"
    );
  });
  ...
  it("suppresses the weight question when no published limit exists", () => {
    const questions = getResultQuestions(
      makeResult("fits", {
        weightLimitKg: null,
        weightVerdict: "not-published",
      })
    );

    expect(
      questions.some((item) => item.questionId === "Q-0002")
    ).toBe(false);
  });
  ...
  it("activates oversize consequences for a failed dimension", () => {
    ...
    expect(
      questions.some((item) => item.questionId === "Q-0003")
    ).toBe(true);
  });
  ...
  it("uses the governed relationship graph rather than category slicing", () => {
    const item = getKnowledgeBySlug(
      "what-happens-if-cabin-bag-is-too-large"
    );
    ...
    expect(
      new Set(related.map((candidate) => candidate.questionId)).size
    ).toBe(related.length);
  });
});