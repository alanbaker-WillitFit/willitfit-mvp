...
describe("RC4 bag artwork contract", () => {
  ...
  it("does not retain the superseded public line-art assets", () => {
    expect(
      fs.existsSync(path.join(iconDirectory, "cabin-bag.svg")),
    ).toBe(false);

    expect(
      fs.existsSync(path.join(iconDirectory, "personal-bag.svg")),
    ).toBe(false);
  });
  ...
  it("uses the locked photographic masters in the bag selector", () => {
    const selector = fs.readFileSync(
      path.join(process.cwd(), "components", "DimensionForm.tsx"),
      "utf8",
    );
