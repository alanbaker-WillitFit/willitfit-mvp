const CACHE="willitlab-four-to-fly-rc1-baseline-1";
const ASSETS=["./","./index.html","./styles.css","./game.js","./manifest.webmanifest","../bag-bounce/asset-refs.js"];
self.addEventListener("install",event=>{event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(ASSETS)));self.skipWaiting();});
self.addEventListener("activate",event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith("willitlab-four-to-fly-")&&key!==CACHE).map(key=>caches.delete(key)))));self.clients.claim();});
