...
function SizeGroupCard({ group }: { group: SizeGuideGroup }) {
  ...
  return (
    <article className={styles.card}>
      ...
      {group.explanatoryLabel ? <small className={styles.explanation}>{group.explanatoryLabel}</small> : null}
      <ul className={styles.airlines}>
        {visible.map((airline) => <li key={airline.airlineId}><Link href={`/airlines/${airline.slug}`}>{airline.airlineName}</Link></li>)}
      </ul>
      {remaining.length > 0 ? (
        <details className={styles.more}>
          <summary>+ {remaining.length} more airlines</summary>
          <ul className={styles.airlines}>
            {remaining.map((airline) => <li key={airline.airlineId}><Link href={`/airlines/${airline.slug}`}>{airline.airlineName}</Link></li>)}
          </ul>
        </details>
      ) : null}
    </article>
...
export default function SizeGuidePage({ title, intro, bagImageSrc, bagImageAlt, imageScale = "standard", sectionTitle, bagTypeQuery, checkerLabel, groups, affiliateSlots, source }: SizeGuidePageProps) {
  ...
  return (
    <div className={`${styles.page} wf-container`}>
      <section className={styles.hero} aria-labelledby="size-guide-title">
        ...
        <div className={`${styles.bagStage} ${imageScale === "zoomed" ? styles.bagStageZoomed : ""}`}>
          <Image src={bagImageSrc} alt={bagImageAlt} fill priority sizes="(max-width: 720px) 90vw, 544px" className={styles.bagImage} />
        </div>
      </section>

      <section aria-labelledby="common-sizes-heading">
        <h2 id="common-sizes-heading" className={styles.sectionTitle}>{sectionTitle}</h2>
        {featuredGroups.length > 0 ? <div className={styles.grid}>{featuredGroups.map((group) => <SizeGroupCard key={group.key} group={group} />)}</div> : (
          <div className={styles.cta}><h2>No published size groups are available yet.</h2><p>Use the checker to review your airline directly.</p></div>
        )}
      </section>

      {additionalGroups.length > 0 ? <details className={styles.more}><summary>Show {additionalGroups.length} more published sizes</summary><div className={styles.grid}>{additionalGroups.map((group) => <SizeGroupCard key={group.key} group={group} />)}</div></details> : null}
