  ...
  return (
    <div className="mt-5">
      <ul className="space-y-2" aria-label={`Airlines using ${group.dimensions.heightCm} by ${group.dimensions.widthCm} by ${group.dimensions.depthCm} centimetres`}>
        {visible.map((airline) => (
          <li key={airline.airlineId}>
            <Link className="font-body text-sm font-semibold text-navy-600 underline decoration-navy-200 underline-offset-4 hover:text-green-600" href={`/airlines/${airline.slug}`}>
              {airline.airlineName}
            </Link>
          </li>
        ))}
      ...
      {remaining > 0 && (
        <button
          type="button"
          className="mt-4 font-body text-sm font-bold text-green-600 underline underline-offset-4"
...
export default function SizeGuideRail({ groups }: { groups: SizeGuideGroup[] }) {
  if (groups.length === 0) {
    return (
      <div className="wf-card mt-8 p-6">
        <p className="font-body text-navy-500">No eligible published dimension groups are currently available.</p>
      </div>
    );
  }
  ...
  return (
    <div
      className="mt-8 flex snap-x snap-mandatory gap-5 overflow-x-auto pb-5"
      aria-label="Published baggage dimension groups"
    >
      {groups.map((group) => (
        <article
          key={group.key}
        ...
        >
          ...
          <h2 className="mt-2 font-heading text-2xl font-bold text-navy-700">
            {group.dimensions.heightCm} × {group.dimensions.widthCm} × {group.dimensions.depthCm} cm
          </h2>
          <p className="mt-2 font-body text-sm text-navy-400">Height × width × depth</p>
